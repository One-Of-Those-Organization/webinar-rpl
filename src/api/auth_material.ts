...
export const auth_material = {
    ...
    add_material: async (data: AddMaterial): Promise<BaseResponse> => {
        try {
            ...
            const response = await fetch(`${API_URL}/api/protected/material-register`, {
                method: "POST",
                headers: {
    ...
    get_material: async (id: number): Promise<BaseResponse> => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`${API_URL}/api/protected/material-info-of?id=${id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
